...
|---|---|---|
| Multiple drives → silently picks first | Resolved — see `USB_Drive_Picker_Reference.md` | Drive picker dialog (#7), done |
| New `.airpack` requires app restart for dropdowns to update | Low — registry reloaded automatically on import; status message tells user to restart | Live dropdown refresh would require `LetterTab` to re-read the registry post-import — not planned |
